...
async function enviarCorreoAdministradores(datosComprador, productos, total, numeroPedido, paymentId) {
  const startTime = Date.now();
  console.log('📧 === INICIANDO ENVÍO DE CORREO A ADMINISTRADORES ===');
  console.log('⏰ Timestamp:', new Date().toISOString());
  ...
  try {
    ...
    // Verificar si hay emails de administradores configurados
    if (!process.env.ADMIN_EMAILS) {
      console.warn('⚠️ No hay emails de administradores configurados (ADMIN_EMAILS)');
      return { success: false, error: 'No hay emails de administradores configurados' };
    }
    ...
    // Verificar conexión SMTP
    console.log('🔍 Verificando conexión SMTP para administradores...');
    await transporter.verify();
    console.log('✅ Conexión SMTP verificada exitosamente');

    // Parsear emails de administradores
    const adminEmails = process.env.ADMIN_EMAILS.split(',').map(email => email.trim());
...
Se ha procesado exitosamente un nuevo pedido:
...
📧 Este correo enviado a administradores
...
Generado automáticamente el ${new Date().toLocaleString('es-AR')}`;
    ...
    const mailOptions = {
      ...
      to: adminEmails,
      subject: `🛒 Nueva Compra #${numeroCorto} - $${parseFloat(total).toFixed(2)} - ${nombreCompleto}`,
      text: emailText
    };

    // Enviar el correo
    console.log('🚀 === ENVIANDO EMAIL A ADMINISTRADORES ===');
    console.log('📧 Enviando a:', adminEmails);
    const info = await transporter.sendMail(mailOptions);
    ...
    console.log('⏱️ Tiempo de envío:', duration + 'ms');
    console.log('📧 Message ID:', info.messageId);
    console.log('✅ Email enviado a administradores:', adminEmails.length, 'destinatarios');
    ...
    try {
      ...
      if (correoCliente) {
        try {
          ...
          try {
            const resultadoEmailAdmin = await enviarCorreoAdministradores(
              { 
                nombre: nombreCompleto,
                email: correoCliente,
                telefono: telefonoCliente,
            ...
            if (resultadoEmailAdmin.success) {
              console.log('✅ Correo a administradores enviado exitosamente desde webhook');
              console.log('📧 Enviado a', resultadoEmailAdmin.recipients, 'administradores');
            } else {
              console.log('⚠️ Error enviando correo a administradores desde webhook:', resultadoEmailAdmin.error);
            }
          } catch (emailAdminError) {
            console.log('⚠️ Excepción enviando correo a administradores desde webhook:', emailAdminError.message);
          }
        } catch (emailError) {
        ...
        try {
          ...
          try {
            ...
            if (resultadoEmailAdmin.success) {
              ...
              console.log('⚠️ Error enviando correo a administradores:', resultadoEmailAdmin.error);
            }
          } catch (emailAdminError) {
            console.log('⚠️ Excepción enviando correo a administradores:', emailAdminError.message);
          }
        } catch (emailError) {
